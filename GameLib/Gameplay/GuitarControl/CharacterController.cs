...
namespace GameLib
{
    ...
    public class CharacterController : Updateable, IBeforeSolverUpdateable
    {
        ...
        public CharacterController()
            : this(new Vector3(), 5.0f, 1.7f * .7f, .7f, 10)
        {
        ...
        void IBeforeSolverUpdateable.Update(float dt)
        {
            ...
            try
            {
                ...
                {
                    dashTimer += dt;
                    HorizontalMotionConstraint.MovementDirection = dashDir;
        ...
        public void Dash(Vector2 dashDirection)
        {
            if (!dashing)
            {
                ...
                force.X = dashDirection.X * dashSpeed * 10;
                force.Z = dashDirection.Y * dashSpeed * 10;

                this.Body.ApplyLinearImpulse(ref force);

                DashingTime = dashTime;
                dashTimer = 0;
                
                dashing = true;
                Stage.ActiveStage.GetQB<PhysicsQB>().startDash();
            }
        }

        public float DashDistOverTime(Vector2 dashDirection, float dist)
        {
            if (!dashing)
            {
                dashDir = Vector2.Zero;
                ...
                vel.X += dashDirection.X * 2 * (dist+10);
                vel.Z += dashDirection.Y * 2 * (dist+10);
                this.Body.LinearVelocity = vel;
                dashVel = vel;
                dashTimer = 0;

                DashingTime = dist / (Math.Abs(vel.Z) + Math.Abs(vel.X));
        ...
        {
            this.Body.LinearVelocity = dashVel;
        }
        ...
        public void FinishDash()
        {
            if (dashing)
            {
                Vector3 vel = this.Body.LinearVelocity;
                vel.X = vel.Z = 0;
                this.Body.LinearVelocity = vel;
                //HorizontalMotionConstraint.Speed = prevSpeed;
                HorizontalMotionConstraint.MovementDirection = Vector2.Zero;
                dashing = false;
                Stage.ActiveStage.GetQB<PhysicsQB>().endDash();
            }
        }
