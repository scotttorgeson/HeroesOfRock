...
namespace GameLib
{
    class RangedEnemyAnimationAgent : Agent
    {
        public RangedEnemyAnimationAgent(Actor actor)
            : base(actor)
        {
            this.Name = "RangedEnemyAnimationAgent";
        ...
        public override void Initialize(Stage stage)
        {
            ...
            if (actor.Parm.HasParm("StartPogoAnimation"))
                animations[(int)AnimationTypes.StartPogo] = new AnimationPlayer(AnimationClip.LoadClip(actor.Parm.GetString("StartPogoAnimation")), modelInstance);
            if (actor.Parm.HasParm("EndPogoAnimation"))
                animations[(int)AnimationTypes.EndPogo] = new AnimationPlayer(AnimationClip.LoadClip(actor.Parm.GetString("EndPogoAnimation")), modelInstance);
            if (actor.Parm.HasParm("PogoAnimation"))
                animations[(int)AnimationTypes.Pogo] = new AnimationPlayer(AnimationClip.LoadClip(actor.Parm.GetString("PogoAnimation")), modelInstance);
