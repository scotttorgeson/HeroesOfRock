...
namespace GameLib
{
    ...
    public class SimpleCharacterController : Updateable, IEndOfTimeStepUpdateable
    {
        ...
        /// <summary>
        /// whether or not the character will collides with the player
        /// </summary>
        ...
        public SimpleCharacterController(Vector3 position, float characterHeight, float radius, float supportHeight, float mass)
        {
            ...
            collisionPairCollector = new Box(position + collisionPairCollectorPositionOffset, radius, supportHeight * 2, radius, 1);
        ...
        private bool FindSupport(out Entity supportEntity, out Vector3 supportLocation, out Vector3 supportNormal, out float supportDistance)
        {
            ...
            for (int i = 0; i < collisionPairCollector.CollisionInformation.Pairs.Count; i++)
            {
                ...
                if((candidate.CollisionRules.Group == PhysicsQB.playerGroup && this.collideWithPlayer == false) || 
                    !PhysicsQB.EnemyRayFilter(candidate.CollisionRules.Group))
                    continue;
